...
def set_conditional_task(
    ...
    role: RoleLiteral,
    description: str,
    condition_type: Literal["price", "cash", "position_value", "position_pnl", "portfolio_value", "position_allocation"],
    ...
    Args:
        ...
        condition_type (required): "price", "cash", "position_value", "position_pnl", "portfolio_value", or "position_allocation".
        comparison (required): "above" or "below".
        threshold (required): Value to compare against. Dollar amounts for prices/values, decimals for percentages (e.g., 0.30 for 30%).
        ticker_symbol (optional): Required for price, position_value, position_pnl, position_allocation conditions.
        related_note_ids (optional): List of note IDs to link to this task. Omit to not link to any notes.
        related_task_ids (optional): List of task IDs to link to this task. Omit to not link to any tasks.
        related_watchlist_ids (optional): List of watchlist IDs to link to this task. Omit to not link to any watchlists.
    """
    requires_ticker = ["price", "position_value", "position_pnl", "position_allocation"]
